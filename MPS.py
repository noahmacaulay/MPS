...
def num_blocks(gamma_list, lmbda_list):
	...
	for i in range(1, len(gamma_list) - 1):
		...
		blocks = find_blocks(temp_alpha, gamma_temp, temp_eta)
...
def block_decomposition(gamma_list_in, lmbda_list_in, i, epsilon = 1e-5):
	...
	blocks = find_blocks(temp_alpha, gamma_list[i], temp_eta)
...
def find_blocks(alpha, M, eta, epsilon = 1e-5):
	...
	M_norms = np.einsum('abi,abi->ab', M, np.conj(M))
